...
- **Session history** — when the agent supports it (Claude does), a history icon button in the header lists previous sessions and resumes the picked one via `session/load`. Hidden when the agent advertises neither `session/list` nor `session/load`.
